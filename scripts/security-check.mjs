...
const requiredPatterns = [
  ...
  ['pinned mirror nodes are removed before rerender', /querySelectorAll\(`\[\$\{MIRROR_ATTRIBUTE\}\]`\)\.forEach\(\(item\) => item\.remove\(\)\)/],
  ['standalone pinned floors receive a mirror', /entry\.pinned && !targetByFloor\.has\(entry\.floor\)[\s\S]*ensurePinnedRootMirror\(entry\)/],
  ['reply chains continue through pinned floors', /targetEntry\.pinned && targetByFloor\.has\(target\)[\s\S]*target = targetByFloor\.get\(target\)/],
  ['pinned threads attach to the root mirror', /rootEntry\.pinned[\s\S]*ensurePinnedRootMirror\(rootEntry\)\.item[\s\S]*rootItem\.append\(thread\)/],
